...
# Solución para el error "Could not get EGL display" en algunos entornos Linux.
# Forzamos el uso del backend de video x11 antes de importar pygame.
import os
...
class Robot3DViewer:
    def __init__(self, width=WINDOW_WIDTH, height=WINDOW_HEIGHT):
        ...
        self.width = width
        self.height = height
        # Inicialización ligera; no arrancamos el render loop aquí.
        self.running = False
        self.thread = None
        ...
        initial_pos = {'x': 0.0, 'y': self.lower_arm_length + self.upper_arm_length + self.effector_length, 'z': 0.0}
        self.robot_position = initial_pos.copy()
        ...
        self.home_position = initial_pos.copy()
        
        # Variables para ejecutar G-code
        self.is_executing_gcode = False
        self.gcode_queue = []
        self.current_position = initial_pos.copy()
    ...
    def update_position(self, x, y, z, animate=True):
        """
        Actualiza la posición objetivo del robot a una coordenada ABSOLUTA.
        Este método siempre interpreta las coordenadas (x, y, z) como un
        destino absoluto en el espacio de trabajo del visualizador.
        ...
        if animate:
            ...
            self.robot_position = {'x': x, 'y': y, 'z': z}
    ...
    def home_robot(self):
        ...
        if not self.motors_enabled:
            ...
            self.update_position(self.home_position['x'], self.home_position['y'], self.home_position['z'])
    ...
    def execute_gcode(self, gcode_commands):
        ...
        # Solución: Sincronizar la posición inicial del G-code con la posición
        # actual del robot en el visualizador para evitar cálculos relativos incorrectos.
    ...
    def _execute_gcode_command(self, command):
        ...
        if main_command == 'G90':
            ...
            self.set_robot_state(effector_active=True)
            ...
            self._move_to_position(self.home_position['x'], self.home_position['y'], self.home_position['z'])
    ...
    def _execute_movement(self, params):
        ...
        if self.coordinate_mode == 'absolute':
            ...
            new_x = x if x is not None else self.current_position['x']
            new_y = y if y is not None else self.current_position['y']
            new_z = z if z is not None else self.current_position['z']
        else:
            # Modo relativo: sumar a la posición actual.
            # Si X o Z no se especifican en el comando, se deben mantener los valores actuales
            # para evitar que la base rote innecesariamente.
            if x is None:
                new_x = self.current_position['x']
            else:
                new_x = self.current_position['x'] + x
            new_y = self.current_position['y'] + (y if y is not None else 0)
            if z is None:
                new_z = self.current_position['z']
            else:
                new_z = self.current_position['z'] + z
        
        # Actualizar la posición actual INMEDIATAMENTE para que el siguiente
        # comando relativo se calcule correctamente.
        self.current_position = {'x': new_x, 'y': new_y, 'z': new_z}
    ...
    def _run_viewer(self):
        ...
        # Estrategia final para "Couldn't find matching GLX visual":
        # Dejar que Pygame/SDL elija la mejor configuración visual disponible
        # y solicitar explícitamente un perfil de compatibilidad de OpenGL.
        pygame.init()
        
        # Solución definitiva: Solicitar un perfil de compatibilidad de OpenGL.
        # Esto es más flexible y tiene más probabilidades de ser soportado por los drivers.
        # Adicionalmente, forzamos una profundidad de color de 16 bits, que es un
        # modo de compatibilidad muy común y suele resolver este error persistente.
        pygame.display.gl_set_attribute(pygame.GL_DEPTH_SIZE, 16)
        pygame.display.gl_set_attribute(pygame.GL_STENCIL_SIZE, 0) # Asegurarse de no pedir stencil buffer
        ...
        try:
            ...
            # No se puede crear el contexto OpenGL (por ejemplo en entornos sin GLX).
            print(f"[robot_3d_viewer] No se pudo inicializar la pantalla OpenGL: {e}")
            # Desactivar el viewer para que no siga intentando renderizar
            self.running = False
            return
        ...
        while self.running:
            ...
            if self.animation_progress < 1.0:
                ...
                smooth_t = self._smooth_step(self.animation_progress)
                for axis in ['x', 'y', 'z']:
                    self.robot_position[axis] = self._lerp(
                        self.robot_position[axis], self.target_position[axis], smooth_t
                    )
    ...
    def _render_target_position(self):
        ...
        target_x, target_y, target_z = self.target_position['x'], self.target_position['y'], self.target_position['z']
