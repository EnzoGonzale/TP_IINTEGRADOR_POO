...
class RobotRpcClient:
    """
    Cliente XML-RPC simplificado, autocontenido en la GUI.
    """
    def __init__(self, endpoint: str = "http://localhost:8080/RPC2", timeout: int = 30):
        # Clase interna para transporte con timeout
        ...
        print(f"Cliente RPC inicializado para el endpoint: {endpoint}")
...
class RobotGUI(tk.Tk):
    def __init__(self):
        ...
        self.client = None # Se inicializará en el login
        self.user = None
        self.password = None
        self.role = None
        self.learning = False
        ...
        # Inicializar visualizador 3D (no instanciamos la clase aquí para evitar
        # intentar abrir un contexto OpenGL en entornos sin servidor X/GLX)
        self.viewer_3d = None
        # Guardamos la referencia a la clase para instanciarla bajo demanda
        self._viewer_class = Robot3DViewer if VIEWER_3D_AVAILABLE else None
    ...
    def create_login(self):
        ...
        ttk.Label(login_frame, text="Servidor RPC:").grid(row=0, column=0, sticky=tk.W, pady=5)
        self.server_entry = ttk.Entry(login_frame, width=30)
        self.server_entry.grid(row=0, column=1, padx=10, pady=5)
        # Valor por defecto y pre-llenado desde variable de entorno
        default_endpoint = os.environ.get("ROBOT_RPC_ENDPOINT", "http://localhost:8080/RPC2")
        self.server_entry.insert(0, default_endpoint)


        ttk.Label(login_frame, text="Usuario:").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.user_entry = ttk.Entry(login_frame, width=30)
        self.user_entry.grid(row=1, column=1, padx=10, pady=5)

        ttk.Label(login_frame, text="Clave:").grid(row=2, column=0, sticky=tk.W, pady=5)
    ...
    def do_login(self):
        ...
        if not server_address or not user or not pwd:
            messagebox.showerror("Login", "Todos los campos son requeridos.")
            return
        try:
            # Crear el cliente justo antes de usarlo con la dirección correcta
            self.client = RobotRpcClient(endpoint=server_address)
    ...
    def create_main_panel(self):
        ...
        if VIEWER_3D_AVAILABLE:
            ...
            ttk.Button(reportsOrTables, text="Detener Visualizador 3D", command=self.stop_gcode_execution).pack(fill=tk.X, pady=2)
    ...
    def on_closing(self):
        # Comprobar si el widget de log existe antes de intentar usarlo.
        # No existirá si se cierra desde la pantalla de login.
        log_exists = hasattr(self, 'log') and self.log.winfo_exists()
        if self.token and log_exists:
            try:
                self.append_log("Cerrando sesión antes de salir...")
                self.client.proxy.user.logout(self.token)
            except Exception as e:
                self.append_log(f"Advertencia durante el logout automático: {e}")
...
# --- Controles ---
    def _ensure_client(self) -> bool:
        if not self.client:
            self.append_log("Error: No está conectado. Realice el login primero.")
            return False
        return True
...
# --- Métodos para el Visualizador 3D ---
    def toggle_3d_viewer(self):
        ...
        if not self._viewer_class:
            messagebox.showerror("Error", "El visualizador 3D no está disponible")
            return

        # Instanciar el visualizador bajo demanda (puede fallar en entornos sin GLX)
        if self.viewer_3d is None:
            try:
                self.viewer_3d = self._viewer_class()
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo inicializar el visualizador 3D: {e}")
                self.viewer_3d = None
                return

        # Verificar que el robot esté conectado antes de permitir abrir el visualizador 3D
        if not self.robot_connected and not getattr(self.viewer_3d, 'running', False):
            messagebox.showerror("Error", "No se puede activar el visualizador 3D: el robot no está conectado")
            return

        if getattr(self.viewer_3d, 'running', False):
            try:
                self.viewer_3d.stop()
            except Exception as e:
                self.append_log(f"Error al detener visualizador 3D: {e}")
            ...
            try:
                ...
                messagebox.showerror("Error", f"Error al iniciar visualizador 3D: {e}")
                self.append_log(f"Error al iniciar visualizador 3D: {e}")
                # Si falla al iniciar, limpiamos la instancia para reintentar después
                try:
                    self.viewer_3d = None
                except Exception:
                    pass
        ...
        try:
            ...
            if method == 'enableMotors':
                ...
                if isinstance(res, dict):
                    ...
                    # Extraer datos de la respuesta (sin bucle innecesario)
                    ...
                    position = res.get('position', {}) # Obtener el diccionario de posición
                    ...
                    if cordinateMode != 'N/A':
                        self.isAbsolute = bool(True if cordinateMode.lower() == 'absoluto' else False)
                    if position != 'N/A':
                    ...
                    # Formatear la posición para el log
                    pos_str = f"X:{position.get('x', 0.0):.2f} Y:{position.get('y', 0.0):.2f} Z:{position.get('z', 0.0):.2f}"
                    self.append_log(f"Posición Actual: {pos_str}")
                    self.append_log("------------------------\n")
                    ...
                    # Actualizar visualizador 3D con la posición actual
                    if position != 'N/A' and isinstance(position, dict):
                        self.update_3d_viewer_position(position.get('x', 0.0), position.get('y', 0.0), position.get('z', 0.0))
                ...
                if res is True:
                    ...
                    if self.motors_enabled:
                        if self.isAbsolute:
                            self.pos['x'] = float(self.x_entry.get())
                            self.pos['y'] = float(self.y_entry.get())
                            self.pos['z'] = float(self.z_entry.get())
                            self.update_3d_viewer_position(self.pos['x'], self.pos['y'], self.pos['z'])
                        else:
                            self.pos['x'] += float(self.x_entry.get())
                            self.pos['y'] += float(self.y_entry.get())
                            self.pos['z'] += float(self.z_entry.get())
